...
namespace My_Utils.Lean_Tween
{
    ...
    public class LeanTweenPath : MonoBehaviour
    {
        ...
        private void init()
        {
            ...
            if (path != null && path.Length != 0 && (pts == null || pts.Length == 0))
            { // transfer over paths made with the legacy path variable
            ...
            if (lastCount != count)
            { // A curve must have been added or subtracted
                ...
                { // BEZIER
                    ...
                    { // remove unused points
                      //Debug.Log("removing stuff count:"+count);
                        ...
                        { // loop through to see if it was any of the in-between nodes deleted (otherwise it will just delete it from the end)
                            if (i % 4 != 0 && pts[i] == null)
                            {
                ...
                { // SPLINE
                    ...
                    k = 0;
                    for (i = 0; i < pts.Length; i++)
                    { // Loop over points to find Transforms that have been deleted
                        if (pts[i] != null)
                        {
                            if (k < newPts.Length)
                            {
