...
{
    /// <summary>
    /// Useful MonoBehaviours functions.
    /// </summary>
    public static class MyUtils
    {
        ...
        public static SpriteRenderer DrawSprite(Color color, Transform parent, Vector2 pos, Vector2 size, string name = null)
        {
            return DrawSprite(null, color, parent, pos, size, name);
        }

        public static SpriteRenderer DrawSprite(Sprite sprite, Transform parent, Vector2 pos, Vector2 size, string name = null)
        {
            return DrawSprite(sprite, Color.white, parent, pos, size, name);
        }

        // Draw a Sprite
        public static SpriteRenderer DrawSprite(Sprite sprite, Color color, Transform parent, Vector2 pos, Vector2 size, string name = null)
        {
            // Setup icon
            if (name == null || name == "") name = "Sprite";
        ...
        /// <summary>
        /// Transform a grid position in a correspondent array position.
        /// </summary>
        ...
        /// <returns></returns>
        public static int PositionToIndex(int x, int y, int gridWidth)
        {
            return x + y * gridWidth;
        }

        /// <summary>
        /// Log a error case itemParameter is null.
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="itemParameter"> A parameter that can't be null </param>
        public static void TreatNullExceptions<T>(T itemParameter, string parameterName, string parentName) where T : class
        {
            if (itemParameter == null)
            {
                Debug.LogError($"Parameter: '{parameterName}' of {parentName} is null.");
            }
        }

        /// <summary>
        /// Returns the atual mouse position converted in wolrd position.
        /// </summary>
        /// <param name="atualCamera">The camera you are using.</param>
        public static Vector2 MouseToWorldPosition(Camera atualCamera)
        {
            return MouseToWorldPosition(Input.mousePosition, atualCamera);
        }

        /// <summary>
        /// Returns a mouse position converted in wolrd position.
        /// </summary>
        /// <param name="atualCamera">The camera you are using.</param>
        public static Vector2 MouseToWorldPosition(Vector2 mousePosition, Camera atualCamera)
        {
            return atualCamera.ScreenToWorldPoint(mousePosition);
        }

        /// <summary>
        /// Transform any index into a valid index of some collection by looping. Ex: list = [1, 2, 3]; LoopIndex(5) => 2;
        /// </summary>
        /// <param name="index">The index that you want to loop.</param>
        /// <param name="collectionLenght">The lenhgt of the collection that you are using.</param>
        /// <returns></returns>
        public static int LoopIndex(int index, int collectionLenght)
        {
            return index - (index / collectionLenght * collectionLenght);
        }

        /// <summary>
        /// Transform any index into a valid index of some collection by clamping. Ex: list = [1, 2, 3]; LoopIndex(5) => 3;
        /// </summary>
        /// <param name="index">The index that you want to loop.</param>
        /// <param name="collectionLenght">The lenhgt of the collection that you are using.</param>
        /// <returns></returns>
        public static int ClampIndex(int index, int collectionLenght)
        {
            if (collectionLenght == 0)
                return -1;

            return Mathf.Clamp(index, 0, collectionLenght - 1);
        }

        private static void DrawArrowUtil(Vector2 fromPos, Vector2 toPos, Vector2 ca_size, float angle, out Vector2 pointA, out Vector2 pointB)
        {
            Vector2 CalculateOffset(Vector2 normalizedDir)
            {
            ...
            Vector2 directionNormalized = dir / dir.magnitude;
            Vector2 arrowAnchor = toPos + (ca_size * -directionNormalized);
            Vector2 adjacent = toPos - arrowAnchor;

            float co_size = Mathf.Sin(angle) * (adjacent.magnitude / Mathf.Cos(angle)) * 0.1f;
            Vector2 offset = CalculateOffset(directionNormalized) * co_size;

            pointA = arrowAnchor + offset;
            pointB = arrowAnchor - offset;
        }

        private static void DrawArrow(Vector2 fromPos, Vector2 toPos, Vector2 ca_size, float angle, DrawType drawType)
        {
            DrawArrowUtil(fromPos, toPos, ca_size, angle, out Vector2 pointA, out Vector2 pointB);

#if UNITY_EDITOR
            if (drawType == DrawType.Gizmos)
            {
                Gizmos.DrawLine(fromPos, toPos);
                Gizmos.DrawLine(pointA, toPos);
                Gizmos.DrawLine(pointB, toPos);
            }
            else // drawType == DrawType.Handles
            {
                Handles.DrawLine(fromPos, toPos);
                Handles.DrawLine(pointA, toPos);
                Handles.DrawLine(pointB, toPos);
            }
#endif
        }

        /// <summary>
        /// Use to draw an arrow with gizmos or handles.
        /// </summary>
        /// <param name="fromPos">The start pos of your line.</param>
        /// <param name="toPos">The end pos of your line.</param>
        /// <param name="ca_size">The size of the adjacent-side of the arrow.</param>
        /// <param name="angle">The angle of the arrow.</param>
        /// <param name="drawType">The draw type.</param>
        public static void DrawArrow(Vector2 fromPos, Vector2 toPos, float ca_size, float angle, DrawType drawType)
        {
            DrawArrow(fromPos, toPos, new Vector2(ca_size, ca_size), angle, drawType);
        }
        ...
        /// <param name="fromPos">The start pos of your line.</param>
        /// <param name="toPos">The end pos of your line.</param>
        /// <param name="ca_percentage">The percentage size of the adjacent-side of the arrow. Proportional to the size of the line.</param>
        /// <param name="angle">The angle of the arrow.</param>
        /// <param name="drawType">The draw type.</param>
        public static void DrawProportionalArrow(Vector2 fromPos, Vector2 toPos, float ca_percentage, float angle, DrawType drawType)
        {
            Vector2 mainLineLenght = fromPos - toPos;
